...
async function loadCourse() {
  ...
  document.getElementById(
    "course"
  ).innerHTML += `<h4><b>${Bitcoincourse} €</b></h4>`;
...
async function loadMonthlyCourse() {
  ...
  console.log(
    responseAsJson["Time Series (Digital Currency Monthly)"]["2024-01-31"][
      "1b. open (USD)"
    ]
  );

  for (let i = 0; i < months.length; i++) {
    const courseEachMonth = Math.round(
      monthlyCourse[months[i]]["1b. open (USD"]
    );
...
function renderChart() {
  ...
  new Chart(ctx, {
    ...
    data: {
      labels: months,
      datasets: [{
        label: 'Bitcoin Course in Dollar',
        data: course,
        borderWidth: 1
      }]
    },
